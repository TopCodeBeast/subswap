...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		/// # </weight>
		#[weight = 0]
		fn issue(origin, issuer: T::AccountId, #[compact] total: T::Balance) -> dispatch::DispatchResult {
			let origin = ensure_signed(origin)?;

			let id = Self::next_asset_id();
			<NextAssetId<T>>::mutate(|id| *id += One::one());

			<Balances<T>>::insert((id, issuer.clone()), total);
		...
		/// # </weight>
		#[weight = 0]
		fn destroy(origin, #[compact] id: T::AssetId) -> dispatch::DispatchResult {
			let origin = ensure_signed(origin)?;
